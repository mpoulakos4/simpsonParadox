...
comparison_plot <- (plot1 | plot2) +
  plot_annotation(
    ...
    subtitle = "The same penguin data reveals opposite relationships when viewed as a whole versus by species",
    caption = "Left: Overall positive trend masks the truth | Right: Within each species, longer bills are actually shallower. This reversal demonstrates how confounding variables (species) can completely invert apparent relationships in aggregated data.",
    theme = theme(
...
This comparison makes Simpson’s Paradox crystal clear - the same data
tells two completely different stories depending on whether we consider
the species grouping variable. The left plot shows the misleading
aggregate relationship, while the right plot reveals the true
